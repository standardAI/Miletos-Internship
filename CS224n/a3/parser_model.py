...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.embed_size*n_features, self.hidden_size)))
    ...
    def forward(self, w):
        ...
        #print(self.embed_to_hidden_weight.shape, self.embed_to_hidden_bias.reshape(1, -1).shape, w.shape, self.embedding_lookup(w).shape)
        first_layer = F.relu(torch.matmul(self.embedding_lookup(w), self.embed_to_hidden_weight) + self.embed_to_hidden_bias.reshape(1, -1))  # (B, H)
        dropout = self.dropout(first_layer)
        logits = torch.matmul(dropout, self.hidden_to_logits_weight) + self.hidden_to_logits_bias.reshape(1, -1)
