...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        init_decoder_hidden = self.h_projection(torch.cat((last_hidden[0], last_hidden[1]), dim=1))  # dim = 1 because we want to
        init_decoder_cell = self.c_projection(torch.cat((last_cell[0], last_cell[1]), dim=1))  # concatenate along the hidden dimension
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, split_size_or_sections=1, dim=0):
            ...
            o_prev = o_t
        #print(combined_outputs[0].size())  # (5, 2)
        combined_outputs = torch.stack(combined_outputs, dim=0)  # (23, 5, 2)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #print(enc_hiddens_proj.shape, dec_hidden.shape)  # (5, 22, 2) (5, 2)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2)  # (5, 22, 1) -> (5, 22)
        ...
        alpha_t = torch.softmax(e_t, dim=1)  # (5, 22)
        #print(alpha_t.shape, enc_hiddens.shape)  # (5, 22) (5, 22, 4)
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)  # (5, 1, 22) (5, 22, 4) -> (5, 4)
        U_t = torch.cat((dec_hidden, a_t), dim=1)  # (5, 6)
        V_t = self.combined_output_projection(U_t)  # (5, 6) -> (5, 2)
