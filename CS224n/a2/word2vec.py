...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    loss = -np.log(softmax(outsideVectors.dot(centerWordVec))[outsideWordIdx])
    
    y_hat = softmax(outsideVectors.dot(centerWordVec))
    gradCenterVec = -outsideVectors[outsideWordIdx].reshape(-1, 1) + outsideVectors.T.dot(y_hat.reshape(-1, 1))
    y_hat[outsideWordIdx] -= 1  # the outside word vector is the true context word vector
    gradOutsideVecs = centerWordVec.reshape(-1, 1).dot(y_hat.reshape(-1, 1).T).T
...
def negSamplingLossAndGradient(
    ...
    for index in negSampleWordIndices:
        loss -= np.log(sigmoid(-outsideVectors[index].dot(centerWordVec)))
    
    gradCenterVec = -outsideVectors[outsideWordIdx]*(1 - sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec)))
    for index in negSampleWordIndices:
        gradCenterVec += outsideVectors[index]*(1 - sigmoid(-outsideVectors[index].dot(centerWordVec)))
    
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = -centerWordVec*(1 - sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec)))
    for index in negSampleWordIndices:
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    #print(len(outsideWords), windowSize)
    ### YOUR CODE HERE (~8 Lines)
    for i in range(2*windowSize):
        outsideWordIdx = word2Ind[outsideWords[i]]
        loss_, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]]\
                                                , outsideWordIdx, outsideVectors, dataset)
        loss += loss_
        #print(gradCenterVecs[word2Ind[currentCenterWord]].shape, gradCenterVec.shape)
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenterVec.reshape(-1, )
