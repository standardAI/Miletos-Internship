...
def svm_loss(x, y):
    ...
    loss = 0.0
    correct_class_scores = x[np.arange(len(x)), y]  # (N,1)
    margin = x - correct_class_scores.reshape(-1,1) + 1  # (N,C) - (N,1) + (1,1) -> (N,C)
    ...
    dx = np.zeros_like(x)
    dx[margin > 0] = 1
    dx[np.arange(len(dx)), y] -= np.sum(dx, axis=1)
    dx /= x.shape[0]
...
def softmax_loss(x, y):
    ...
    softmaxes = np.exp(x) / denominators.reshape(-1, 1)
    softmaxes[range(x.shape[0]), y] -= 1
    dx = softmaxes
    dx /= x.shape[0]
    #dW += reg * 2 * W
