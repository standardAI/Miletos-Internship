...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            self.dependencies += [(self.stack[-1], self.stack.pop(-2))]
        elif transition == "RA":
            #print(self.stack, self.buffer, self.dependencies)
            self.dependencies += [(self.stack[-2], self.stack.pop())]
...
#        mini_batch = unfinished_parses[i:i + batch_size]
        mini_batch = unfinished_parses[:batch_size]
        transitions = model.predict(mini_batch)
        for mb, t in zip(mini_batch, transitions):
            mb.parse([t])
#            if len(mb.buffer) == 0 and len(mb.stack) == 1:
#                unfinished_parses.remove(mb)
#        i += batch_size
        
        for sentence in mini_batch:
            if len(sentence.buffer) == 0 and len(sentence.stack) == 1:
                unfinished_parses.remove(sentence)
#        if i + batch_size > len(unfinished_parses):
#            i = 0
    
    for sent in partial_parses:
